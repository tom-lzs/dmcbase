$PBExportHeader$w_a_udis_pt.srw
$PBExportComments$[MAIN] Ancêtre des fenêtres de Gestion Mono-ligne
forward
global type w_a_udis_pt from w_a
end type
type dw_1 from u_dw_udis within w_a_udis_pt
end type
end forward
...
global w_a_udis_pt w_a_udis_pt
...
// In your descendant, include the following:
...
on ue_delete;call w_a::ue_delete;// Set FALSE if and only if we get all the way through the script
i_b_canceled = TRUE

IF dw_1.fu_delete(1) THEN
	i_b_canceled = FALSE
END IF
...
on ue_cancel;call w_a::ue_cancel;i_b_canceled = TRUE  // tell closequery not to ask about saving
...
IF fw_ask_update ("New anyway?") = 3 THEN
	RETURN
END IF
...
on ue_insert;call w_a::ue_insert;// This is a single record maintenance object--
// use "ue_new" instead.
end on
