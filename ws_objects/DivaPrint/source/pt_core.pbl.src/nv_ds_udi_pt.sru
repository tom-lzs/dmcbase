$PBExportHeader$nv_ds_udi_pt.sru
$PBExportComments$Datastore Ancêtre pour la mise à jour
forward
global type nv_ds_udi_pt from nv_dsa
end type
end forward

global type nv_ds_udi_pt from nv_dsa
end type
global nv_ds_udi_pt nv_ds_udi_pt
...
IF IsValid (fu_get_prior ()) THEN
	po_head = this.fu_get_head_po()
	RETURN po_head.function dynamic fu_update ()
END IF
...
IF IsValid (i_po_next) THEN
	IF NOT i_po_next.function dynamic fu_check_required () THEN
...
IF IsValid (i_po_next) THEN
	IF NOT i_po_next.function dynamic fu_validate () THEN
		i_nv_dw_dbi.fnv_set_update_status (-8)
		RETURN FALSE
	END IF
END IF
...
IF IsValid (i_po_next) THEN
	IF NOT i_po_next.function dynamic fu_accepttext () THEN
		i_nv_dw_dbi.fnv_set_update_status (-8)
		RETURN FALSE
	END IF
END IF
...
// capture the transaction object specified for descendents of u_dsa.
//
// This objects 'super' which captures the transaction
// object.
...
i_nv_dw_dbi.fnv_set_parent (po_this)
...
on nv_ds_udi_pt.create
call datastore::create
TriggerEvent( this, "constructor" )
end on

on nv_ds_udi_pt.destroy
call datastore::destroy
TriggerEvent( this, "destructor" )
end on

event sqlpreview;call super::sqlpreview;// Call the event processor on the DataWindow database interface object.
...
event ue_predelete;call super::ue_predelete;// Confirm delete if the switch is set.  Set message.returnvalue to -1
