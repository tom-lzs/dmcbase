...
if dw_1.RowCount() > 0 then
    ...
    for l_row = 1 to dw_1.RowCount() 
			...
			ds_datastore.settrans( SQLCA)
			if ds_datastore.retrieve(dw_1.getItemString(l_row,"artae000") , dw_1.getItemString(l_row,"dimaeart")) > 0 then
				dw_1.SetItem(l_row,"stkdispo" ,ds_datastore.GetItemDecimal(1,"stkaedis")	)
			end if

			ds_datastore.dataobject = "d_ctrl_relicats"  
			ds_datastore.settrans( SQLCA)
			if ds_datastore.retrieve(i_nv_commande_object.fu_get_client_donneur_ordre(), dw_1.getItemString(l_row,"artae000") , dw_1.getItemString(l_row,"dimaeart"))  > 0 then
				dw_1.SetItem(l_row,"relicat" ,ds_datastore.GetItemDecimal(1,"relicat")	)
			end if	
   next
end if
