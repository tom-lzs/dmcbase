...
forward prototypes
protected function string fnv_construct_path (string a_s_dir, string a_s_file)
...
protected function integer fnv_parse_filenames (ref string a_s_file[])
end prototypes
...
//		use a_s_dir to construct full path.
//		(This function called by fnv_parse_filenames.)
//	Arguments:
//		string	a_s_dir		Directory name (without trailing '\')
...
ELSE
	...
	IF s_first_char = "." THEN				//relative to parent directory
		...
		// if not found assume a_s_dir is volume
		IF i_slash_pos = 0 THEN
			s_parent = a_s_dir
		ELSE
			// don't include slash
			s_parent = Mid (a_s_dir, 1, i_slash_pos - 1)
		END IF
	...
	ELSEIF s_first_char = "\" THEN			// relative to root directory
		...
		s_path = Mid (a_s_dir, 1, 2) + a_s_file

	ELSE									// relative to current directory

		s_path = a_s_dir + "\" + a_s_file
...
protected function integer fnv_parse_filenames (ref string a_s_file[]);
//*****************************************************************************
// Parse filenames return by GetOpenFileNames
//	Arguments:
//		string	- REF a_s_file[] an unbounded array of strings to hold filenames
//	Returns
//		int		- i_count the count of filenames
//*****************************************************************************

string	s_dir, s_file, s_path
string	s_empty_array[]
int		i_idx, i_pos, i_len, i_start
int		i_count

// initialize array and cound
a_s_file = s_empty_array
i_count = 0

// check that we have filenames to parse
IF IsNull (i_s_files) OR i_s_files = "" THEN
	RETURN i_count
END IF

// if no space character found, only 1 file selected
i_pos = Pos (i_s_files, " ", 1)
IF i_pos = 0 THEN
	i_count = 1
	a_s_file[i_count] = i_s_files
	RETURN i_count
END IF

// need a space on end for loop to work
i_s_files = i_s_files + " "

// directory starts the list
s_dir = Mid (i_s_files, 1, i_pos - 1)

i_start = i_pos + 1
i_pos = Pos (i_s_files, " ", i_start)

// search for space delimited filenames until end of string
DO WHILE i_pos > 0

	// build full pathname of file
	i_len = i_pos - i_start
	s_file = Mid (i_s_files, i_start, i_len)
	s_path = this.fnv_construct_path (s_dir, s_file)

	IF s_path <> "" THEN
		i_count ++
		a_s_file[i_count] = s_path
	END IF
...
// return the count of valid files
RETURN i_count
end function
